...
void printHelp(const po::options_description& desc)
{
    ...
    std::cout << "and generates professional budget analysis spreadsheets." << std::endl
              << std::endl;
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (verbose) {
            ...
            for (const auto& a : accountNames)
                std::cout << a << " ";
            std::cout << std::endl;
            std::cout << "Output file: " << outputFile << std::endl;
            std::cout << "Format: " << format << std::endl;
            std::cout << "Generate Spreadsheet: " << (generateSpreadsheet ? "Yes" : "No")
                      << std::endl;
            std::cout << "=============================================" << std::endl << std::endl;
        }
        ...
        if (vm.count("category-config")) {
            ...
            if (fileExists(configPath)) {
                if (verbose) {
                    std::cout << "Loading custom category configuration from: " << configPath
                              << std::endl;
                }
            ...
            if (fileExists(defaultConfig)) {
                if (verbose) {
                    std::cout << "Loading category configuration from: " << defaultConfig
                              << std::endl;
                }
        ...
        for (size_t i = 0; i < inputFiles.size(); ++i) {
            if (verbose) {
                std::cout << "Processing: " << inputFiles[i] << " (Account: " << accountNames[i]
                          << ")..." << std::endl;
            }
            ...
            try {
                ...
                if (verbose) {
                    std::cout << "  OK Loaded " << transactions.size() << " transactions"
                              << std::endl;
                }
        ...
        if (verbose) {
            std::cout << "\nTotal transactions loaded: " << totalTransactions << std::endl
                      << std::endl;
        }
        ...
        for (const auto& cat : summary.categoryBreakdown) {
            std::cout << "  " << std::setw(20) << std::left << cat.first << ": $" << std::setw(10)
                      << std::right << std::fixed << std::setprecision(2) << cat.second
                      << std::endl;
        }
        ...
        if (verbose) {
            ...
            std::cout << "Average Monthly Spending: $" << analyzer.getAverageMonthlySpending()
                      << std::endl;
        }
