...
MoneyTrackerGUI::MoneyTrackerGUI()
    : window(nullptr), main_box(nullptr), notebook(nullptr), file_list(nullptr),
      account_list(nullptr), output_entry(nullptr), category_config_entry(nullptr),
      summary_text(nullptr), category_text(nullptr), monthly_text(nullptr),
      transactions_text(nullptr), chat_input(nullptr), chat_output(nullptr), chat_send_btn(nullptr),
      ollama_status_label(nullptr), model_combo(nullptr), progress_bar(nullptr),
      status_label(nullptr), transaction_data(std::make_shared<TransactionData>()),
      config_manager(std::make_shared<ConfigManager>()), ai_assistant(nullptr)
{
...
void MoneyTrackerGUI::build_ui()
{
    ...
    const char* css_styles =
        ...
        ".header-label { font-size: 14pt; font-weight: bold; color: #333; margin: 10px 0px 5px "
        "0px; }\n"
    ...
    gtk_notebook_append_page(GTK_NOTEBOOK(notebook), input_page, gtk_label_new("📁 Input Files"));
    ...
    GtkWidget* info_label =
        gtk_label_new("Use categories.json to define custom transaction categorization rules.\n"
    ...
    GtkWidget* info_inner =
        gtk_label_new("💡 Tip: Add your own patterns to categories.json for better categorization");
    ...
    gtk_notebook_append_page(GTK_NOTEBOOK(notebook), config_page, gtk_label_new("⚙️ Config"));

    // ========== TAB 3: Summary Results ==========
    GtkWidget* summary_scrolled = gtk_scrolled_window_new(NULL, NULL);
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(summary_scrolled), GTK_POLICY_AUTOMATIC,
                                   GTK_POLICY_AUTOMATIC);
    ...
    gtk_notebook_append_page(GTK_NOTEBOOK(notebook), summary_scrolled, gtk_label_new("📈 Summary"));

    // ========== TAB 4: Category Breakdown ==========
    GtkWidget* category_scrolled = gtk_scrolled_window_new(NULL, NULL);
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(category_scrolled), GTK_POLICY_AUTOMATIC,
                                   GTK_POLICY_AUTOMATIC);
    ...
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(monthly_scrolled), GTK_POLICY_AUTOMATIC,
                                   GTK_POLICY_AUTOMATIC);
    ...
    gtk_notebook_append_page(GTK_NOTEBOOK(notebook), monthly_scrolled, gtk_label_new("📅 Trends"));

    // ========== TAB 6: Transactions ==========
    GtkWidget* trans_scrolled = gtk_scrolled_window_new(NULL, NULL);
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(trans_scrolled), GTK_POLICY_AUTOMATIC,
                                   GTK_POLICY_AUTOMATIC);
    ...
    gtk_widget_set_tooltip_text(analyze_btn,
                                "Analyze selected CSV files and generate report (Ctrl+A)");
    gtk_widget_set_size_request(analyze_btn, 140, 40);

    GtkWidget* excel_btn =
        gtk_button_new_from_icon_name("document-open-recent", GTK_ICON_SIZE_BUTTON);
    ...
    gtk_widget_add_accelerator(analyze_btn, "clicked", accel_group, GDK_KEY_a, GDK_CONTROL_MASK,
                               GTK_ACCEL_VISIBLE);
    gtk_widget_add_accelerator(excel_btn, "clicked", accel_group, GDK_KEY_o, GDK_CONTROL_MASK,
                               GTK_ACCEL_VISIBLE);
    gtk_widget_add_accelerator(quit_btn, "clicked", accel_group, GDK_KEY_q, GDK_CONTROL_MASK,
                               GTK_ACCEL_VISIBLE);
    gtk_widget_add_accelerator(add_btn, "clicked", accel_group, GDK_KEY_n, GDK_CONTROL_MASK,
                               GTK_ACCEL_VISIBLE);
}
...
void MoneyTrackerGUI::on_add_file(GtkWidget* widget, gpointer data)
{
    ...
    GtkWidget* dialog = gtk_file_chooser_dialog_new(
        "Select CSV File", GTK_WINDOW(self->window), GTK_FILE_CHOOSER_ACTION_OPEN, "_Cancel",
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
        ...
        GtkWidget* name_dialog = gtk_dialog_new_with_buttons(
            "Enter Account Name", GTK_WINDOW(self->window), GTK_DIALOG_MODAL, "_OK",
            GTK_RESPONSE_OK, "_Cancel", GTK_RESPONSE_CANCEL, NULL);

        GtkWidget* entry = gtk_entry_new();
        gtk_entry_set_text(GTK_ENTRY(entry), "Account");
        gtk_container_add(GTK_CONTAINER(gtk_dialog_get_content_area(GTK_DIALOG(name_dialog))),
                          entry);
...
void MoneyTrackerGUI::on_choose_output(GtkWidget* widget, gpointer data)
{
    ...
    GtkWidget* dialog = gtk_file_chooser_dialog_new(
        "Save Excel File", GTK_WINDOW(self->window), GTK_FILE_CHOOSER_ACTION_SAVE, "_Cancel",
...
void MoneyTrackerGUI::on_choose_config(GtkWidget* widget, gpointer data)
{
    ...
    GtkWidget* dialog = gtk_file_chooser_dialog_new(
        "Select Category Config", GTK_WINDOW(self->window), GTK_FILE_CHOOSER_ACTION_OPEN, "_Cancel",
        GTK_RESPONSE_CANCEL, "_Open", GTK_RESPONSE_ACCEPT, NULL);
...
void MoneyTrackerGUI::update_file_list()
{
    ...
    for (size_t i = 0; i < input_files.size(); ++i) {
        ss << (i + 1) << ". " << input_files[i].first << " [" << input_files[i].second << "]\n";
    }
...
void MoneyTrackerGUI::display_results()
{
    ...
    // Category tab
    {
        ...
        for (const auto& cat : summary.categoryBreakdown) {
            ss << std::left << std::setw(25) << cat.first << ": $" << std::fixed
               << std::setprecision(2) << cat.second << "\n";
        }
    ...
    // Monthly tab
    {
        ...
        for (const auto& month : summary.monthlyTrends) {
            ss << month.first << ": $" << std::fixed << std::setprecision(2) << month.second
               << "\n";
        }
    ...
    {
        std::stringstream ss;
        ss << std::left << std::setw(12) << "Date" << " | " << std::setw(30) << "Description"
           << " | " << std::setw(12) << "Amount" << " | " << std::setw(15) << "Category" << " | "
           << std::setw(15) << "Account" << "\n";
        ss << std::string(90, '-') << "\n";

        for (const auto& trans : transaction_data->getAllTransactions()) {
            ss << std::left << std::setw(12) << trans.date << " | " << std::setw(30)
               << trans.description.substr(0, 27) << " | " << std::setw(12) << std::fixed
               << std::setprecision(2) << trans.amount << " | " << std::setw(15) << trans.category
...
void MoneyTrackerGUI::show_error(const std::string& message)
{
    ...
    GtkWidget* dialog =
        gtk_message_dialog_new(GTK_WINDOW(window), GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR,
...
void MoneyTrackerGUI::show_info(const std::string& message)
{
    ...
    GtkWidget* dialog =
        gtk_message_dialog_new(GTK_WINDOW(window), GTK_DIALOG_MODAL, GTK_MESSAGE_INFO,
                               GTK_BUTTONS_OK, "%s", message.c_str());
...
void MoneyTrackerGUI::build_chat_tab()
{
    ...
    gtk_button_set_image(GTK_BUTTON(test_btn),
                         gtk_image_new_from_icon_name("network-server", GTK_ICON_SIZE_BUTTON));
    ...
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(chat_scrolled), GTK_POLICY_AUTOMATIC,
                                   GTK_POLICY_AUTOMATIC);
    ...
    gtk_button_set_image(GTK_BUTTON(chat_send_btn),
                         gtk_image_new_from_icon_name("mail-send", GTK_ICON_SIZE_BUTTON));
    ...
    const char* quick_questions[] = {"Top spending categories?", "Monthly trends?",
                                     "Unusual transactions?", "Budget advice?"};
    ...
    // Add to notebook
    gtk_notebook_append_page(GTK_NOTEBOOK(notebook), chat_page, gtk_label_new("🤖 AI Assistant"));
...
void MoneyTrackerGUI::update_ollama_status()
{
    if (moneytracker::AIAssistant::checkOllamaInstalled()) {
        ...
        if (!models.empty()) {
            std::string status =
                "🟢 Ollama ready - " + std::to_string(models.size()) + " model(s) available";
...
void MoneyTrackerGUI::on_chat_send(GtkWidget* widget, gpointer data)
{
    ...
    MoneyTrackerGUI* self =
        static_cast<MoneyTrackerGUI*>(g_object_get_data(G_OBJECT(clicked_widget), "gui"));
    ...
    if (self->ai_assistant) {
        moneytracker::AIResponse response =
            self->ai_assistant->chatWithContext(question, *self->transaction_data);
